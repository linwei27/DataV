...
<body>
	<div class="x-nav">
		<span class="layui-breadcrumb"> <a href="javascript:void(0);">首页</a>
			<a href="javascript:void(0);">功能列表</a> <a><cite>抓捕人员照片</cite></a>
	...
	<div>
		<select name="type" onchange="sel(this.options[selectedIndex].text)">
			...
			<option value="1">未成年犯罪照片</option>
			<!-- 默认显示此值 -->
			<option value="2" selected="selected">抓捕人员照片</option>
					...
					<div class="layui-card">
						...
						<div class="layui-card-header">
							<span class="layui-btn"
								onclick="xadmin.open('导入文件','toUpload?type=14',800,600)" /> <i
							...
							<table class="layui-table layui-form">
								...
								<tbody>
									<c:forEach items="${page.list}" var="item" varStatus="st">
										<tr>
											<td>${st.index + 1 }</td>
											<td><img src="show?fileName=${item.destFileName}&type=14"
...
<script>
	layui.use([ 'laydate', 'form' ], function() {
		var form = layui.form;
		//监听提交
		form.on('submit(byzbPhotoList)', function(data) {
			return false;
		});
	});
</script>
